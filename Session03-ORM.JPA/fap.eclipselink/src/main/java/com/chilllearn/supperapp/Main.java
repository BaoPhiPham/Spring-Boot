...
public class Main {
    private static final EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.chilllearn.eclipselink-PU");
...
//        List<Lecturer> lecturers = em.createQuery("SELECT x FROM Lecturer x WHERE x.salary = :pSalary", Lecturer.class).setParameter("pSalary", 20_000_000) .getResultList();
    ...
    public static void getAllLecturers(){
        EntityManager em = emf.createEntityManager();
        //luôn cần có ô quản lý các Entity
...
//        List<Lecturer> lecturers = em.createQuery("SELECT x FROM Lecturer x WHERE x.salary = 20000001", Lecturer.class).getResultList();//so sánh số trong DB thì nên viết chuẩn số
        List<Lecturer> lecturers = em.createQuery("SELECT x FROM Lecturer x", Lecturer.class).getResultList();
...
//        emf.close();//sau getAll mà ko làm gì nửa thì close luôn connection, ko close thì tốn RAM
    ...
    public static void getAllStudents(){
        EntityManager em = emf.createEntityManager();
        List<Student> students = em.createQuery("SELECT s FROM Student s", Student.class).getResultList();//hậu trường là SELECT * FROM Student của SQL truyền thống - sẽ in ra xem nếu khai báo cấu hình trong file xml
